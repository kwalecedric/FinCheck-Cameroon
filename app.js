...
function applyUptimeData(monitors) {
  monitors.forEach(monitor => {
    // Match monitor friendly_name to our service names
    ...
    if (monitor.status === 2)      match.status = "operational";
    else if (monitor.status === 8) match.status = "degraded";
    else if (monitor.status === 9) match.status = "outage";

    // Update note with live uptime ratio
...
const translations = {
  en: {
    ...
    nav_logs:        "Live Logs",
    signout:         "Sign out",
    live_data:       "Fetching live uptime data...",
    live_data_ok:    "Live data from UptimeRobot",
    ...
    select_issue:    "Type of issue...",
    issue_login:     "Cannot log in",
    issue_tx:        "Transaction failed",
    issue_funds:     "Funds debited but not received",
    issue_otp:       "OTP not received",
    issue_app:       "App not loading",
    issue_slow:      "Slow transactions",
    issue_other:     "Other",
    desc_placeholder:"Describe the issue briefly (optional)...",
    submit_report:   "Submit report",
    report_ok:       "✓ Report submitted. Thank you!",
    ...
    uptime_chart:    "Uptime % — Last 30 days",
    incident_chart:  "Incident frequency",
    heatmap_title:   "Peak downtime hours",
    heatmap_sub:     "Hours when outages occur most",
    response_title:  "Response time trends",
    response_sub:    "Average response time (ms)",
    tracker_title:   "Transaction Tracker",
    tracker_sub:     "Monitor and manage your transactions",
    total:           "Total",
    total_sent:      "Total sent",
    search_tx:       "Search transactions...",
    all:             "All",
    success:         "Success",
    pending:         "Pending",
    failed:          "Failed",
    ...
    some_issues:     "Some disruptions",
    some_down:       "Some services down",
    last_30:         "Last 30 days",
    newly_added:     "Newly added service",
    add_ok:          "added to your services.",
    add_err:         "Please fill in both fields.",
    remove_ok:       "Service removed.",
    dispute_ok:      "✓ Dispute submitted. Reference: ",
    dispute_err:     "Failed to submit.",
    dispute_reason_err: "Please select a reason.",
    health_great:    "Excellent",
    health_good:     "Good",
    health_poor:     "Poor",
    health_prefix:   "Health: ",
    reports_count:   "community report",
    reports_counts:  "community reports",
    founded_by:      "Built by Ngong Kwale Cedric",
  },
  fr: {
    brand:           "FinCheck Cameroun",
    nav_overview:    "Aperçu",
    nav_monitoring:  "Surveillance",
    nav_tracker:     "Suivi",
    nav_services:    "Mes Services",
    nav_logs:        "Journaux Live",
    signout:         "Déconnexion",
    live_data:       "Récupération des données en direct...",
    live_data_ok:    "Données en direct via UptimeRobot",
    greeting_prefix: "Bienvenue",
    overview_title:  "Aperçu de la plateforme",
    ...
    select_issue:    "Type de problème...",
    issue_login:     "Impossible de se connecter",
    issue_tx:        "Transaction échouée",
    issue_funds:     "Fonds débités mais non reçus",
    issue_otp:       "OTP non reçu",
    issue_app:       "Application ne charge pas",
    issue_slow:      "Transactions lentes",
    issue_other:     "Autre",
    desc_placeholder:"Décrivez brièvement le problème (optionnel)...",
    submit_report:   "Soumettre le signalement",
    report_ok:       "✓ Signalement soumis. Merci!",
    ...
    tracker_title:   "Suivi des transactions",
    tracker_sub:     "Surveillez et gérez vos transactions",
    total:           "Total",
    total_sent:      "Total envoyé",
    search_tx:       "Rechercher une transaction...",
    all:             "Tout",
    success:         "Succès",
    pending:         "En attente",
    failed:          "Échoué",
    ...
    type_placeholder:"Type (ex: Banque Mobile)",
    ...
    dispute_title:   "Contester une transaction",
    dispute_reason:  "Raison de la contestation",
    select_reason:   "Sélectionner une raison...",
    r1:              "Fonds débités mais non reçus",
    r2:              "Montant incorrect débité",
    r3:              "Transaction non initiée par moi",
    r4:              "Service non rendu",
    r5:              "Autre",
    details:         "Détails supplémentaires",
    details_ph:      "Décrivez ce qui s'est passé...",
    submit_dispute:  "Soumettre la contestation",
    timeline_title:  "Chronologie de la transaction",
    refresh_in:      "Actualisation dans",
    refresh_btn:     "Actualiser maintenant",
    ...
    all_good:        "Tous les systèmes opérationnels",
    some_issues:     "Quelques perturbations",
    some_down:       "Certains services indisponibles",
    last_30:         "30 derniers jours",
    newly_added:     "Service nouvellement ajouté",
    add_ok:          "ajouté à vos services.",
    add_err:         "Veuillez remplir les deux champs.",
    remove_ok:       "Service supprimé.",
    dispute_ok:      "✓ Contestation soumise. Référence: ",
    dispute_err:     "Échec de la soumission.",
    dispute_reason_err: "Veuillez sélectionner une raison.",
    health_great:    "Excellent",
    health_good:     "Bon",
    health_poor:     "Faible",
    health_prefix:   "Santé: ",
    reports_count:   "signalement communautaire",
    reports_counts:  "signalements communautaires",
    founded_by:      "Créé par Ngong Kwale Cedric",
  }
};
...
async function loadUserServices() {
  ...
  if (snap.exists()) {
    userServices = snap.val();
  } else {
    await set(ref(db, `users/${currentUser.uid}/services`), defaultServices);
  }
  populateReportSelect();
  renderOverview();
}
...
function populateReportSelect() {
  ...
  if (sel) {
    sel.innerHTML = `<option value="">${t("select_service")}</option>` +
  ...
  // Update report section heading and subtitle
...
function renderServices() {
  ...
  grid.innerHTML = filtered.map(s => {
    ...
    const msg    = hoverMessages[s.status]?.[currentLang] || hoverMessages[s.status]?.en || "";
...
window.submitReport = async function() {
  ...
  if (!service || !type) {
    feedback.textContent = t("report_err"); feedback.className = "feedback error"; return;
  }
  try {
    ...
    feedback.textContent = t("report_err"); feedback.className = "feedback error";
  }
};
...
function renderIncidentChart() {
  const el = document.getElementById("chart-incidents");
  if (!el) return;
  const months = currentLang==="fr"
    ? ["Nov","Déc","Jan","Fév","Mar","Avr"]
    : ["Nov","Dec","Jan","Feb","Mar","Apr"];
  const data = [2,1,4,2,3,5];
...
function renderHeatmap() {
  ...
  const values = [8,5,3,2,4,6,7,5,9,12,10,7];
  const max    = Math.max(...values);
  el.innerHTML =
    `<div class="heatmap">` +
    values.map(v => {
      const intensity = v/max;
      const bg = `rgba(231,76,60,${0.1+intensity*0.8})`;
      return `<div class="heatmap-cell" style="background:${bg}">${v}</div>`;
    }).join("") + `</div>
    <div class="heatmap-labels">` +
    hours.map(h => `<div class="heatmap-label">${h}h</div>`).join("") +
    `</div>`;
}

function renderResponseChart() {
  const el = document.getElementById("chart-response");
  if (!el) return;
  const svcs   = ["Sara","MTN","Orange","Express","CCA","Yoomee"];
  const values = [420,180,210,890,160,240];
  const max    = Math.max(...values);
  ...
  el.innerHTML = sorted.map(([name,amt],i)=>`
    <div class="chart-row">
      ...
      <div class="chart-bar-bg">
        <div class="chart-bar-fill" style="width:${(amt/max)*100}%;background:${colors[i%colors.length]}"></div>
      </div>
...
function renderTransactions() {
  ...
  list.innerHTML = filtered.map(tx=>{
    const statusLabel = tx.status==="success"?t("successful"):tx.status==="pending"?t("pending"):t("failed");
    const disputeBtn  = tx.status==="failed"
      ? `<button class="tx-btn danger" onclick="openDispute('${tx.id}')">${t("dispute")}</button>` : "";
    const timelineBtn = tx.status==="pending"
      ? `<button class="tx-btn" onclick="openTimeline('${tx.id}')">${t("timeline")}</button>` : "";
    return `
      <div class="tx-card">
        <div class="tx-main">
          <div class="tx-info">
          ...
          <div class="tx-right">
            ...
            <span class="badge ${tx.status}">${statusLabel}</span>
          </div>
        </div>
...
window.submitDispute = async function() {
  ...
  try {
    ...
    fb.textContent = t("dispute_ok") + currentDisputeTx.id;
    fb.className = "feedback success";
    setTimeout(()=>window.closeDispute(), 3000);
  } catch(e) {
    fb.textContent=t("dispute_err"); fb.className="feedback error";
  }
};
...
window.openTimeline = function(txId) {
  ...
  document.getElementById("timeline-content").innerHTML = `
    <p style="font-size:13px;color:rgba(255,255,255,0.5);margin-bottom:1.25rem">
      ${tx.to} · ${tx.amount.toLocaleString()} XAF</p>
    <div class="timeline">
      ${steps.map(s=>`
...
function renderManageServices() {
  const el = document.getElementById("services-manage");
  if (!el) return;
...
function startLiveLogs() {
  ...
  onValue(logsRef, snap => {
    ...
    const data = snap.val();
    if (!data) {
      list.innerHTML=`<div class="no-logs">${t("no_logs")}</div>`;
    ...
    if (countEl) countEl.textContent =
      `${reports.length} ${reports.length!==1?t("reports_counts"):t("reports_count")}`;
